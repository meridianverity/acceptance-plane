# Changelog

## v1.0.2-public-eval - 2026-07-05

- Restored dotfiles missing from the submitted ZIP so manifest verification passes from a fresh archive.
- Added exact release title, release description, GitHub release body, and pointer lock.
- Updated public release commands from v1.0.0 historical flow to v1.0.2-public-eval.
- Added strategic disclosure boundary documentation.
- Updated proof receipt, SBOM, release attestation, QA report, and manifest metadata for v1.0.2-public-eval.


## v1.0.1-public-eval - 2026-06-11

- Added hardened public-evaluation proof overlay.
- Fixed manifest/source-tree drift from the uploaded ZIP package.
- Restored missing `.github`, `.gitignore`, and `.zenodo.json` files.
- Added deterministic 64-vector ACCEPT/HOLD/REFUSE scenario suite.
- Added scenario-card linter promised by the README.
- Added Ed25519-signed public-eval proof receipt.
- Added signed Merkle transparency bundle.
- Added independent recomputation script that does not import the evaluator package.
- Added source/archive release artifact verifier.
- Added Makefile, pyproject packaging metadata, pytest suite, schema examples, and adapter-shape sketches.
- Strengthened public claims discipline and release gate checks.

## v1.0.0 - 2026-06-11

- Public canonical package for the Acceptance Plane.
- Embedded canonical Zenodo DOI: https://doi.org/10.5281/zenodo.20645907.
- Finalized "before impact" canonical definition.
- Updated short-form slogan to "real-world consequence."
- Polished the acceptance-boundary wording in the whitepaper.
- Replaced pre-publication DOI-insertion workflow with DOI-embedded final release assets.
- Set GitHub-Zenodo archive metadata to `upload_type: software` while preserving the manual Zenodo technical report DOI as the canonical citation.
- Replaced draft wording with release wording.
- Added polished, non-clipped conceptual diagrams with canonical DOI.
- Standardized license metadata to CC BY 4.0.
- Added professional DOCX/PDF metadata.
- Added SHA256 manifest and verification script.
